...
with open("data.json", "r") as file:
    ...
    for block in blocks:
        res = apis[current_api := (current_api + 1) % 10].instrumentMarket("ISIN", block)
        for i in range(len(block)):
            instruments = set(map(lambda x: x["bc"], res["data"]["instruments"][i]["referenceData"]["instrumentMarkets"]))
            cres[block[i]] = list(instruments)
            print(instruments.intersection(available_bcs))

with open("possible_bcs.json", "w") as file:
    json.dump(cres, file)
